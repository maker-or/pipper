...
const Accordion = forwardRef<HTMLDivElement, AccordionProps>((props, ref) => {
  ...
  const defaultValue = (rest as any).defaultValue;
  const value = (rest as any).value;
  const onValueChange = (rest as any).onValueChange;
  ...
  const radixProps: any = { type };
  if (type === "multiple") {
    const v = (value as string[] | undefined) ?? internalMultipleValue;
    if (v !== undefined) radixProps.value = v;
    const dv = defaultValue as string[] | undefined;
    if (dv !== undefined) radixProps.defaultValue = dv;
    radixProps.onValueChange = handleMultipleChange;
  } else {
    if (collapsible !== undefined) radixProps.collapsible = collapsible;
    const v = (value as string | undefined) ?? internalSingleValue;
    if (v !== undefined) radixProps.value = v;
    const dv = defaultValue as string | undefined;
    if (dv !== undefined) radixProps.defaultValue = dv;
    radixProps.onValueChange = handleSingleChange;
  }
